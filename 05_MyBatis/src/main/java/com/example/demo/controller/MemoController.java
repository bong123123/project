...
//	}
	...
	@PostMapping("/add")
	public String memo_post(@ModelAttribute @Valid MemoDto memoDto, BindingResult bindingResult, Model model) throws Exception {
		log.info("POST /memo/post..." + memoDto);
...
//			model.addAttribute("id",bindingResult.getFieldError("id").getDefaultMessage());
		...
		boolean success = memoServiceImpl.memoRegistration(memoDto);
		if(success) {
			return "/home";
		}else {
			return "/memo/add";
